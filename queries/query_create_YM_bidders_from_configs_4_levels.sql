...
cohort_bidders as (
    ...
    from cohorts c
    left join `streamamp-qa-239417.DAS_increment.{YM_strategy_name}_domain_{tablename_ext_YM_config}` l3
        using (date, domain)
    left join `streamamp-qa-239417.DAS_increment.{YM_strategy_name}_geo_continent_country_code_{tablename_ext_YM_config}` l2
        using (date, geo_continent, country_code)
    left join `streamamp-qa-239417.DAS_increment.{YM_strategy_name}_geo_continent_{tablename_ext_YM_config}` l1
